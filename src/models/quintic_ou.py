...
class QuinticOU(BaseModel):
    ...
    def set_parameters(self, eps: float, H: float, a_k: list, t_array_nodes: list, fv_nodes: list, rho: float, S0: float, spline_order: int = 3):
        self.eps = eps
        self.H = H
    ...
    def generate_vol_trajectory(self, tt: np.ndarray, Xt: np.ndarray):
        ...
        fv_var_curve_spline_sqrt = interpolate.splrep(self.t_array_nodes, np.sqrt(self.fv_nodes), k=self.spline_order)
    ...
    def generate_underlying_trajectories(self, time_step: int, w1: np.ndarray, volatility: np.ndarray):
        logS0 = np.log(self.S0)
        logSt = logS0 + np.cumsum(- 0.5 * time_step * (volatility * self.rho) ** 2 + np.sqrt(time_step) * self.rho * volatility * w1, axis=0)
